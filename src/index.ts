...
(async () => {
  ...
  if (process.env.BOT_TOKEN && process.env.BOT_CLIENT_ID) {
    ...
    (process.env.DISCORD_TOKEN_WIKISUBMISSION &&
      process.env.DISCORD_CLIENTID_WIKISUBMISSION) ||
    (process.env.DISCORD_TOKEN_WIKISUBMISSION &&
      process.env.DISCORD_CLIENTID_WIKISUBMISSION)
  ) {
    console.log(
      `Environment variables loaded (using WikiSubmission ${process.env.NODE_ENV === "production" ? "production" : "development"} credentials)\n`
    );
  } else {
    console.error(
