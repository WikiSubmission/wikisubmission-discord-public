...
export function logError(
  ...
  if (error instanceof DiscordAPIError) {
    ...
    console.error(
      `[${context}] DiscordAPIError at ${source}: ${error.message}`
    );
    console.error(`Status: ${error.status}, Code: ${error.code}`);
    if (error.requestBody?.json) {
      console.error(
        "Request Body:",
        JSON.stringify(error.requestBody.json, null, 2)
      );
    }
...
function getCallerInfo(): string {
  ...
  const callerLine =
    lines.find((line) => !line.includes("logDiscordError")) || lines[0];
